...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fetch and display user's agencies
  fetch("api/get_user_agencies.php")
    .then((res) => res.json())
    .then((response) => {
      if (response.status === "success") {
        ...
        if (agencies.length === 0) {
          container.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>You haven’t created any agencies yet.</p>
                    </div>`;
          return;
        }
        ...
        <div class="agency-card">
            <div class="agency-image">
                <img src="${
                  agency.image || "assets/default-agency.jpg"
                }" alt="${agency.name}">
            </div>
            ...
            <div class="agency-actions">
                <button class="see-btn" onclick="window.location.href='agency.html?agency_id=${
                  agency.id
                }'">See</button>
                <button class="dashboard-btn" onclick="window.location.href='api/dashboard/AgencyDash/index.php?agency_id=${agency.id}'">Dashboard</button>
    ...
    `;
        ...
        console.error("Error loading agencies:", response.message);
      }
    })
    .catch((error) => {
      ...
      if (response.status === "success") {
        ...
        messages.forEach((msg) => {
          ...
          item.className = `message-item ${
            msg.status === "pending" ? "status-pending" : "status-answered"
          }`;

          item.innerHTML = `
          <h3>${msg.subject}</h3>
          <p class="message-date">${new Date(
            msg.submitted_at
          ).toLocaleString()}</p>
  ...
  document.querySelectorAll(".inner-tab-link").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      document
        .querySelectorAll(".inner-tab-link")

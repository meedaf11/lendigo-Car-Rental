...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function switchTab(tabId) {
    ...
    if (tabId === "paymentTab") {
      ...
      if (!isNaN(carPrice) && carPrice > 0) {
        const minPayment = Math.ceil(carPrice * 0.1);
        document.getElementById(
          "paymentAmount"
        ).placeholder = `Minimum: ${minPayment} DH`;
  ...
  const stripe = Stripe(
    "pk_test_51QnVHRG278XaJnqxLnI0O2LhX29NlknG5827TOqBy6krhTOaSaa43md7ReiY78akJdPG2mAOqF71J3koJ064iaDR00B59FXxgs"
  ); // Replace with your real test/live key
  ...
  function validateFields(fields) {
    ...
    if (fields.includes("paymentAmount")) {
      ...
      const carPrice = parseFloat(document.getElementById("carPrice").value);
      const minPayment = Math.ceil(carPrice * 0.1); // rounded up for clarity
      ...
      if (actualPayment < minPayment) {
        alert(
          `Minimum registration fee must be at least 10% of daily price: ${minPayment} DH`
        );
      ...
      if (result.error) {
        ...
        try {
          ...
          if (saveData.success) {
            ...
            alert(
              "Payment succeeded but failed to save agency: " + saveData.error
            );
          }
        } catch (err) {
